...
function hasBlockedImageHost(value) {
  if (typeof value !== 'string') return false;
  const s = value.trim();
  if (!s) return false;
  ...
  try {
    const url = new URL(s);
    return blockedHostPatterns.some((pattern) => pattern.test(url.hostname));
  } catch {
    return /tiktok/i.test(s);
  }
}

function isBlockedImageCandidate(img) {
  if (!img || typeof img !== 'object') return false;

  if (typeof img.source === 'string' && /tiktok/i.test(img.source)) {
    return true;
  }

  return [img.url, img.original_url, img.thumbnail_url].some(hasBlockedImageHost);
}

function selectReviewImageUrl(img) {
  if (!img || typeof img !== 'object') return '';

  const candidates = [img.thumbnail_url, img.url, img.original_url];

  for (const candidate of candidates) {
    if (!isValidHttpUrl(candidate)) continue;
    if (hasBlockedImageHost(candidate)) continue;
...
for (const img of rawImages) {
  ...
  if (isBlockedImageCandidate(img)) {
    skippedImages.push({
      raw_url: url || null,
  ...
  const reviewImageUrl = selectReviewImageUrl({
    url,
...
const candidateList = selectedImages
  .map((img, i) => `${i + 1}. ${img.review_image_url}`)
  .join('\n');
...
const content = [
  ...
  ...selectedImages.map(img => ({
    type: 'input_image',
    image_url: img.review_image_url,
    detail: 'low'
  }))
];
